/**
 * Convert ZeeWorld ROA Excel file to JSON format (Final Correct Version)
 * 
...
function convertROAFile() {
  ...
  try {
    console.log('🚀 Starting ROA Excel to JSON conversion (Final Correct Version)...');
    
    // Convert the Excel file
    const roaData = convertROAExcel(filePath);
    
    // Save to JSON file
    const outputPath = path.join(__dirname, '../public/roa-guide-final.json');
    ...
    const copyPath = path.join(__dirname, '../public/roa-guide-copy-final.json');
