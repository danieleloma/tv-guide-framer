...
function createLocalISO(dateISO, time, timezone) {
  ...
  const result = `${dateTimeStr}${timezoneOffset}`;
  console.log(`Creating ISO: ${dateISO}T${time}:00${timezoneOffset} = ${result}`);
  return result;
}
...
function convertROAExcelUltimate() {
  console.log('🚀 Starting ROA Excel to JSON conversion (Ultimate Version)...');
  ...
  dataRows.forEach((row, index) => {
    ...
    if (index < 5) {
      console.log(`Row ${index + 1}: ${title} - ${startTime} -> ${startTimeStr}, ${endTime} -> ${endTimeStr} (${timezone})`);
    }
  ...
  for (let i = 0; i < 7; i++) {
    ...
    daySlots.sort((a, b) => {
      // Parse the time from the ISO string for sorting
      const aTime = a.startISO.match(/T(\d{2}):(\d{2}):/);
      const bTime = b.startISO.match(/T(\d{2}):(\d{2}):/);
  ...
  // Write to file
  const outputPath = './public/roa-guide-ultimate.json';
