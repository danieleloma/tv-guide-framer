/**
 * Perfect Excel to JSON Converter
 * 
...
function convertSAExcelPerfect(filePath) {
...
function convertROAExcelPerfect(filePath) {
...
function convertExcelFilesPerfect() {
  const args = process.argv.slice(2);
  
  if (args.length < 2) {
    console.log(`
Usage: node convert-excel-perfect.js <sa-excel-file> <roa-excel-file>

Example:
  node convert-excel-perfect.js "Zee World OCTOBER 25 FPC SA.xlsx" "Zee World OCTOBER 25 FPC ROA.xlsx"
...
with all 7 days and correct show titles/times.
  ...
  console.log('🚀 Starting Perfect Excel to JSON conversion...');
  ...
  if (fs.existsSync(saFile)) {
    saData = convertSAExcelPerfect(saFile);
    if (saData) {
      fs.writeFileSync('./public/sa-guide-perfect.json', JSON.stringify(saData, null, 2));
      console.log('✅ SA guide data written to ./public/sa-guide-perfect.json');
    }
  } else {
  ...
  if (fs.existsSync(roaFile)) {
    roaData = convertROAExcelPerfect(roaFile);
    if (roaData) {
      fs.writeFileSync('./public/roa-guide-perfect.json', JSON.stringify(roaData, null, 2));
      console.log('✅ ROA guide data written to ./public/roa-guide-perfect.json');
    }
  } else {
  ...
  console.log('\n📋 Next Steps:');
  console.log('1. Copy the JSON content from sa-guide-perfect.json');
  console.log('2. Paste it into the "SA Data JSON" field in Framer');
  console.log('3. Copy the JSON content from roa-guide-perfect.json');
  console.log('4. Paste it into the "ROA Data JSON" field in Framer');
}

// Run the conversion
convertExcelFilesPerfect();
