...
function convertROAExcelExact() {
  console.log('🚀 Starting ROA Excel to JSON conversion (Exact Format)...');
  ...
  // Write to file
  const outputPath = './public/roa-guide-exact.json';
  ...
  if (days.length > 0 && days[0].slots.length > 0) {
    days[0].slots.slice(0, 3).forEach((slot, index) => {
