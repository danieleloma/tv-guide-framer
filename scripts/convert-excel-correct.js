/**
 * Correct Excel to JSON Converter
 * 
...
// Helper function to convert decimal hours to proper time string
...
function timeToUTCISO_SA(dateISO, timeStr) {
  ...
  // Create date in CAT timezone (UTC+2)
  const dateTime = new Date(`${dateISO}T${timeStr}:00+02:00`);
...
function timeToUTCISO_ROA_WAT(dateISO, timeStr) {
  ...
  // Create date in WAT timezone (UTC+1)
  const dateTime = new Date(`${dateISO}T${timeStr}:00+01:00`);
...
function convertSAExcelCorrect(filePath) {
...
function convertROAExcelCorrect(filePath) {
...
function convertExcelFilesCorrect() {
  const args = process.argv.slice(2);
  
  if (args.length < 2) {
    console.log(`
Usage: node convert-excel-correct.js <sa-excel-file> <roa-excel-file>

Example:
  node convert-excel-correct.js "Zee World OCTOBER 25 FPC SA.xlsx" "Zee World OCTOBER 25 FPC ROA.xlsx"
...
with all 7 days and correct show titles/times.
  ...
  console.log('🚀 Starting Correct Excel to JSON conversion...');
  ...
  if (fs.existsSync(saFile)) {
    saData = convertSAExcelCorrect(saFile);
    if (saData) {
      fs.writeFileSync('./public/sa-guide-correct.json', JSON.stringify(saData, null, 2));
      console.log('✅ SA guide data written to ./public/sa-guide-correct.json');
    }
  } else {
  ...
  if (fs.existsSync(roaFile)) {
    roaData = convertROAExcelCorrect(roaFile);
    if (roaData) {
      fs.writeFileSync('./public/roa-guide-correct.json', JSON.stringify(roaData, null, 2));
      console.log('✅ ROA guide data written to ./public/roa-guide-correct.json');
    }
  } else {
  ...
  console.log('\n✅ Correct conversion completed!');
  console.log('\n📋 Next Steps:');
  console.log('1. Copy the JSON content from sa-guide-correct.json');
  console.log('2. Paste it into the "SA Data JSON" field in Framer');
  console.log('3. Copy the JSON content from roa-guide-correct.json');
  console.log('4. Paste it into the "ROA Data JSON" field in Framer');
}

// Run the conversion
convertExcelFilesCorrect();
