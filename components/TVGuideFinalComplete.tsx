/**
 * Final Complete TV Guide Framer Component
 * 
 * Features:
 * - Split shows based on timezones and regions
 * - Dynamic access across different time zones and regions
 * - Time on horizontal axis, days/dates on vertical axis
 * - Manual JSON data insertion for SA and ROA
 * - Manual toggle visibility for regions
 * - 7 day calendar (Monday-Sunday)
 * - No time conversions (local time)
 * - WAT: 5am-4am, CAT: 6am-5am (within 24 hour cycle)
 * - All TV shows align with allotted time slots
 * - 30-minute interval blocks with consistent width
 */
...
function generateSATimeTicks(hourWidthPx: number) {
    const ticks = []
    const halfHourWidth = hourWidthPx / 2 // 30-minute blocks are half width
    ...
    for (let hour = 6; hour <= 23; hour++) {
        ...
        if (hour < 23) {
            ticks.push({
                ...
                xPosition: (hour - 6) * hourWidthPx + halfHourWidth,
                isMajor: false,
            })
        }
    }
    ...
    for (let hour = 0; hour <= 5; hour++) {
        ...
        if (hour < 5) {
            ticks.push({
                ...
                minute: 30,
                label: `${hour.toString().padStart(2, "0")}:30`,
                xPosition: (hour + 18) * hourWidthPx + halfHourWidth,
                isMajor: false,
            })
        } else {
            // 5:30 AM (last tick)
            ticks.push({
                ...
                minute: 30,
                label: "05:30",
                xPosition: (5 + 18) * hourWidthPx + halfHourWidth,
                isMajor: false,
            })
        }
    }
    ...
    for (let hour = 5; hour <= 23; hour++) {
        ...
        if (hour < 23) {
            ticks.push({
                ...
                minute: 30,
                label: `${hour.toString().padStart(2, "0")}:30`,
                xPosition: (hour - 5) * hourWidthPx + halfHourWidth,
                isMajor: false,
            })
        }
    }
    ...
    for (let hour = 0; hour <= 4; hour++) {
        ...
        if (hour < 4) {
            ticks.push({
                ...
                minute: 30,
                label: `${hour.toString().padStart(2, "0")}:30`,
                xPosition: (hour + 19) * hourWidthPx + halfHourWidth,
                isMajor: false,
            })
        }
    }
    ...
    const combinedData = useMemo(() => {
        ...
        return {
            metadata: {
                ...
                defaultRegion: regions.length > 0 ? regions[0].code : "ROA",
                defaultTimezone: "WAT",
            },
            regions,
        }
                                        ...
                                        <div
                                            ...
                                            style={{
                                                width: tick.isMajor
                                                    ...
                                                    : (props.hourWidthPx || 220) / 2, // 30-minute blocks are half width
                                                padding: "12px 8px",
                                                textAlign: "center",
...
addPropertyControls(TVGuideFinalComplete, {
    // JSON inputs for both regions
    saDataJSON: {
